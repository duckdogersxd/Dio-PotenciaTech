...
insert into orders (idOrderClient, orderStatus, orderDescription, sendValue, paymentCash) values 
                             ...
                             (5,default,'compra via web site',null,1),
                             (6,default,'compra via aplicativo',100,0);
...
select c.idClient, COUNT(o.idOrder) AS orderCount from clients c 
...
SELECT DISTINCT S.idSeller, S.SocialName AS NomeVendedor, S.CNPJ AS CPNJVendedor,
                F.SocialName AS NomeFornecedor, F.CNPJ AS CNPJFornecedor
FROM seller S
INNER JOIN productSeller PS ON S.idSeller = PS.idPseller
INNER JOIN productSupplier PSup ON S.idSeller = PSup.idPsSupplier
INNER JOIN supplier F ON PSup.idPsSupplier = F.idSupplier;
